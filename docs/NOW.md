...
- 上一轮普通浏览器复验因画面几乎全深蓝、缺乏方向和空间参照，无法判断移动是否生效；本节点增加明确初始朝向、三枚技术诊断参照物、分段路径标尺和低频诊断 HUD。它们只是灰盒可观察性占位，不是正式林肯岛美术或地名。
- 用户在第 2.6 复验时连静态 HUD 也未看见；服务端已确认包含最新源码，但客户端陈旧页面或访问版本仍待通过可见 build id 定位，不能把它断言为唯一缓存根因。
