...
- **本地开发入口：** Docker Desktop、明确版本的 Node 24.18.0 镜像、npm `package-lock.json` 和 Docker 命名卷；开发进程以非 root 用户运行。
...
- Babylon.js、React、Vite、测试工具等依赖的精确版本。
- 首轮浏览器矩阵、输入方式和性能预算。
- 第一人称还是第三人称作为默认漫游方式。
- Terrain、高度图或自定义网格的地形方案。
- 坐标尺度、岛屿压缩比例及快速旅行策略。
